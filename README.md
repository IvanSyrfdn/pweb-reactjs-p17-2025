...
Status: Backend: disediakan (lihat query params di bookRoutes.js).      
Frontend: pencarian & genre-filter sudah ada/di-wired (sebagaimana Anda/ saya edit BookListPage), pagination UI perlu dikonfirmasi (backend mendukung pagination lewat page/limit).      
...
### Langkah Selanjutnya
#### 1. Proteksi AddBook dan halaman admin
Pastikan ada halaman Login/Register di frontend. Setelah login, simpan token dan gunakan ProtectedRoute wrapper untuk AddBookPage, Delete actions, dan halaman transaksi.
#### 2. Transaksi
Buat backend /api/transactions (POST untuk checkout, GET untuk history, GET/:id).
Buat frontend pages: Checkout, Transactions list, Transaction detail.
#### 3. Pagination UI
Jika BookList belum punya control untuk naik/turun halaman, tambahkan komponen pagination yang memanggil backend ?page=...&limit=....
#### 4. Tes End-To-End
Jalankan backend (npm start), frontend (npm run dev), lakukan alur: login → add book (upload image) → cek BookList (cover muncul) → buka detail → hapus buku.
#### 5. Opsional Polish
Hero/caratousel, font, spacing, hover, wishlist/ratings untuk mendekati tampilan Gramedia.